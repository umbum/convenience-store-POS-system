...
public class OrderHistoryRepoImpl implements OrderHistoryRepo {
    ...
    public List<OrderHistory> readAll(String date, long branchId) {
        ...
        String selectOrder = "SELECT DISTINCT ORDER_ID, BRANCH_ID, ORD_DATE AS ORDER_DATE, AMOUNT FROM A_ORDER WHERE ORD_DATE = TRUNC(?) AND BRANCH_ID = ?";
        String selectCompanyOrders = "SELECT DISTINCT C.COMPANY_ID, C.COM_NAME AS COMPANY_NAME\n"
            + "FROM PRODUCT P, ORDER_PRODUCT OP, COMPANY C\n"
            + "WHERE OP.ORDER_ID = ? AND OP.PRODUCT_ID = P.PRODUCT_ID AND P.COMPANY_ID = C.COMPANY_ID";
